# Package metadata.
TITLE       := MicroPython PS4 REPL
VERSION     := 1.00
TITLE_ID    := BREW00053
CONTENT_ID  := IV0000-BREW00053_00-MICROPYTHON00000
...
# Exclude modsys.c as we disabled MICROPY_PY_SYS
MP_PY_FILES := $(filter-out micropython_embed/py/modsys.c, $(wildcard micropython_embed/py/*.c))
MP_SHARED_FILES := $(wildcard micropython_embed/shared/runtime/*.c)
...
MP_INCLUDES := -I. -Imicropython_embed -Imicropython_embed/port
CFLAGS      := --target=x86_64-pc-freebsd12-elf -fPIC -funwind-tables -c $(EXTRAFLAGS) -isysroot $(TOOLCHAIN) -isystem $(TOOLCHAIN)/include $(MP_INCLUDES)
CXXFLAGS    := $(CFLAGS) -isystem $(TOOLCHAIN)/include/c++/v1
LDFLAGS     := -m elf_x86_64 -pie --script $(TOOLCHAIN)/link.x --eh-frame-hdr -L$(TOOLCHAIN)/lib $(LIBS) $(TOOLCHAIN)/lib/crt1.o

# Create the intermediate directory incase it doesn't already exist.
_unused     := $(shell mkdir -p $(INTDIR))
_unused2    := $(shell mkdir -p $(INTDIR)/micropython_embed/port)
_unused3    := $(shell mkdir -p $(INTDIR)/micropython_embed/py)
_unused4    := $(shell mkdir -p $(INTDIR)/micropython_embed/shared/runtime)
